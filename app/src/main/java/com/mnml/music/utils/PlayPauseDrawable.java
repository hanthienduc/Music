...
public class PlayPauseDrawable extends Drawable {
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        canvas.rotate(
                lerp(startingRotation, startingRotation + 90f, rotationProgress), width / 2f, height / 2f);

        // Position the pause/play button in the center of the drawable's bounds.
        canvas.translate(
                Math.round(width / 2f - ((2f * barWidth + barDist) / 2f)),
                Math.round(height / 2f + (pauseBarHeight / 2f)));
    ...
    private Animator getPausePlayAnimator() {
        isPlaySet = !isPlaySet;
        final Animator anim =
                ObjectAnimator.ofFloat(this, PROGRESS, isPlay ? 1f : 0f, isPlay ? 0f : 1f);
        anim.addListener(
