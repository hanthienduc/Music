...
import com.mnml.music.task.FetchArtistImg;
...
public abstract class ArtistImgHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        String CREATE_PLAYBACK_SONG_TABLE =
                "CREATE TABLE "
                        + TABLE_PLAYBACK
                        + " ("
                        + ARTIST_KEY_ID
    ...
    public String getArtistArtWork(final String name, int pos) {
        ...
        if (url != null) {
            ...
            else removeArtistImgFromDB(name);
        } else {
    ...
    private void removeArtistImgFromDB(String name) {
        SQLiteDatabase db = this.getWritableDatabase();
        String query =
                "DELETE  FROM " + TABLE_PLAYBACK + " WHERE " + ARTIST_KEY_NAME + "='" + name + "'";
