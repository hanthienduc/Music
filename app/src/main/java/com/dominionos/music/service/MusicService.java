  ...
  private void playMusic(final Song song) {
    ...
    if (result == AudioManager.AUDIOFOCUS_REQUEST_GRANTED) {
      ...
      try {
        ...
        Toast.makeText(MusicService.this, getString(R.string.file_invalid), Toast.LENGTH_SHORT)
            .show();
      }
    } else {
      Toast.makeText(MusicService.this, getString(R.string.unable_gain_focus), Toast.LENGTH_SHORT)
          .show();
    }
  ...
  private Notification createNotification() {
    PendingIntent playIntent =
        PendingIntent.getBroadcast(
            this,
            100,
            new Intent(Config.TOGGLE_PLAY).setPackage(getPackageName()),
            PendingIntent.FLAG_UPDATE_CURRENT);
    PendingIntent prevIntent =
        PendingIntent.getBroadcast(
            this,
            100,
            new Intent(Config.PREV).setPackage(getPackageName()),
            PendingIntent.FLAG_UPDATE_CURRENT);
    PendingIntent nextIntent =
        PendingIntent.getBroadcast(
            this,
            100,
            new Intent(Config.NEXT).setPackage(getPackageName()),
            PendingIntent.FLAG_UPDATE_CURRENT);
    PendingIntent cancelIntent =
        PendingIntent.getBroadcast(
            this,
            100,
            new Intent(Config.CANCEL_NOTIFICATION).setPackage(getPackageName()),
            PendingIntent.FLAG_UPDATE_CURRENT);
    PendingIntent launchAppIntent =
        PendingIntent.getActivity(
            this,
            100,
            new Intent(getApplicationContext(), MainActivity.class).setPackage(getPackageName()),
            PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    NotificationCompat.Action prev =
        new NotificationCompat.Action(
            R.drawable.ic_skip_previous, getString(R.string.previous), prevIntent);
    NotificationCompat.Action playPause;
    notificationBuilder.addAction(prev);
    if (isPlaying) {
      playPause =
          new NotificationCompat.Action(R.drawable.ic_pause, getString(R.string.play), playIntent);
    } else {
      playPause =
          new NotificationCompat.Action(R.drawable.ic_play, getString(R.string.play), playIntent);
    }
    notificationBuilder.addAction(playPause);
    if (playingList.size() > 1) {
      NotificationCompat.Action next =
          new NotificationCompat.Action(
              R.drawable.ic_skip_next, getString(R.string.next), nextIntent);
      notificationBuilder.addAction(next);
    }
    NotificationCompat.Action cancel =
        new NotificationCompat.Action(
            R.drawable.ic_remove, getString(R.string.cancel), cancelIntent);
